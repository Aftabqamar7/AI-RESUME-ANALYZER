...
class ResumeSchema(BaseModel):
    ...
    skills: List[str] = Field(default_factory=list, description="List of detected technical and soft skills")
    total_experience_years: float = Field(default=0.0, description="Total years of work experience detected")
    education_level: Optional[str] = Field(default=None, description="Highest degree attained")
    section_headings: List[str] = Field(default_factory=list, description="Headings found in CV")
    has_contact_info: bool = Field(default=True, description="True if phone/email is present")
...
class JobDescriptionSchema(BaseModel):
    ...
    required_experience_years: float = Field(default=0.0, description="Minimum required experience years")
    required_education_level: Optional[str] = Field(default=None, description="Required education level")
    keywords: List[str] = Field(default_factory=list, description="Important domain keywords")


class AIAdviceSchema(BaseModel):
    recommendations: List[str] = Field(default_factory=list, description="Top actionable improvements")
    rewritten_bullet_suggestions: List[str] = Field(default_factory=list, description="Suggested bullet points")
...
class ResumeAnalyzerEngine:
    ...
    def generate_recommendations(self, missing_skills: List[str], score_breakdown: Dict[str, Any]) -> Dict[str, Any]:
        prompt = f"""
        Based on these evaluation metrics, generate top 4 actionable resume improvements.
    ...
    def calculate_matches_and_scores(self, resume_data: Dict[str, Any], jd_data: Dict[str, Any], raw_cv_text: str) -> Dict[str, Any]:
        ...
        exp_score = 100.0 if jd_exp == 0 else min(100.0, (cv_exp / jd_exp) * 100)
